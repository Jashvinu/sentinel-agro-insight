/**
 * Earth Engine Utilities for Firebase Cloud Functions
 * Uses Firebase secrets for authentication
 */
...
/**
 * Build service account credentials from Firebase secrets or environment
 */
export function buildServiceAccount(): any {
    // Try Firebase secrets first (set via firebase functions:secrets:set)
    const privateKey = (
    ...
    const clientEmail =
        ...
        'wrkfarm-415118@appspot.gserviceaccount.com';
    ...
    if (!privateKey) {
        throw new Error(
            'Missing GOOGLE_PRIVATE_KEY. Set it using: firebase functions:secrets:set GOOGLE_PRIVATE_KEY'
        );
    }
...
/**
 * Initialize Earth Engine using environment variables or Firebase secrets
 */
