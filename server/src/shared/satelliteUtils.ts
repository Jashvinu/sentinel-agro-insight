...
// Ported from Deno/Supabase Edge Functions to Node.js/Express
...
export async function getAllOpticalDates(
  ...
  startDate: string,
  endDate: string,
  evaluateFn: (obj: any) => Promise<any>,
  ...
  console.log(`[SatelliteUtils] Querying ${satellites.length} optical satellites for dates between ${startDate} and ${endDate}`);

  for (const { config, name } of satellites) {
    try {
      console.log(`[SatelliteUtils] Querying ${name}...`);
      ...
      const imageList = await evaluateFn(
      ...
      for (const img of imageList) {
        if (!img || !img.properties) {
          console.warn(`[SatelliteUtils] Skipping invalid image data from ${name}`);
          continue;
        }
        ...
        if (entry.date && entry.timestamp > 0) {
          ...
          console.warn(`[SatelliteUtils] Skipping invalid entry from ${name}:`, entry);
        }
      }

      console.log(`[SatelliteUtils] ${name}: Found ${count} observations`);
    } catch (error: any) {
      console.error(`[SatelliteUtils] Error querying ${name}:`, error?.message || error);
  ...
  console.log(`[SatelliteUtils] Total optical observations: ${allDates.length}`);
...
export async function getSentinel1Dates(
  ...
  try {
    console.log(`[SatelliteUtils] Querying Sentinel-1 SAR...`);
    ...
    const imageList = await evaluateFn(collection.aggregate_array('.all'));
    ...
    for (const img of imageList) {
      if (!img || !img.properties) {
        console.warn(`[SatelliteUtils] Skipping invalid SAR image data`);
        continue;
      }
      ...
      if (entry.date && entry.timestamp > 0) {
        ...
        console.warn(`[SatelliteUtils] Skipping invalid SAR entry:`, entry);
      }
    }

    console.log(`[SatelliteUtils] Sentinel-1 SAR: Found ${dates.length} observations`);
    return dates;
  } catch (error: any) {
    console.error(`[SatelliteUtils] Error querying Sentinel-1 SAR:`, error?.message || error);
...
export async function getAllSatelliteDates(
  ...
  console.log(`[SatelliteUtils] Fetching dates from all satellites (${startDate} to ${endDate})`);

  // Fetch optical and SAR dates in parallel for better performance
  const [opticalDates, sarDates] = await Promise.all([
    getAllOpticalDates(poi, startDate, endDate, evaluateFn, maxCloudCover),
    getSentinel1Dates(poi, startDate, endDate, evaluateFn)
  ]);
  ...
  console.log(`[SatelliteUtils] Total observations from all satellites: ${combined.length}`);
  console.log(`[SatelliteUtils] Breakdown:`, satelliteCounts);
 ...
 */
export function getDataSourceSummary(collection: any, evaluateFn: (obj: any) => Promise<any>): Promise<any> {
  return evaluateFn(
