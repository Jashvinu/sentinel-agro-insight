...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    for (const index of indices) {
      ...
      // Calculate index for the composite
      ...
      // For trend detection, compare first and last images
      let trend = 0;
      ...
      try {
        ...
        const firstIndex = calculator(firstImage);
        const lastIndex = calculator(lastImage);
