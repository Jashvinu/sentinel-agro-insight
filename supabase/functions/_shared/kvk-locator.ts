/**
 * KVK Locator — nearest Krishi Vigyan Kendra lookup for Maharashtra farms.
 * Static table built from ICAR-ATARI Pune (Zone-VIII) contact records.
 * No public API exists; distance is computed client-side with Haversine.
 */
...
// Maharashtra KVKs — ATARI Pune Zone-VIII + Zone-IX
export const MAHARASHTRA_KVKS: KVKRecord[] = [
  // Vidarbha — rice belt
  ...
  { name: 'KVK Gadchiroli', district: 'Gadchiroli', division: 'Vidarbha', lat: 20.181, lng: 80.009, crops: ['rice', 'minor_millets'], source_id: 'kvk-gadchiroli' },
...
export function findNearestKVKs(lat: number, lng: number, count = 3): KVKWithDistance[] {
  return MAHARASHTRA_KVKS
    .map((k) => ({ ...k, distance_km: Math.round(haversineM(lat, lng, k.lat, k.lng) / 100) / 10 }))
    .sort((a, b) => a.distance_km - b.distance_km)
    .slice(0, count);
}

/** Find the most relevant KVK for a given crop */
export function findKVKByCrop(lat: number, lng: number, crop: string): KVKWithDistance | null {
  const cropLower = crop.toLowerCase();
  const matching = MAHARASHTRA_KVKS
    .filter((k) => k.crops.some((c) => c.toLowerCase().includes(cropLower) || cropLower.includes(c.toLowerCase())))
    .map((k) => ({ ...k, distance_km: Math.round(haversineM(lat, lng, k.lat, k.lng) / 100) / 10 }))
    .sort((a, b) => a.distance_km - b.distance_km);

  return matching[0] ?? findNearestKVKs(lat, lng, 1)[0] ?? null;
}
