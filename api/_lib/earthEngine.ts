/**
 * Earth Engine Utilities for Vercel API Routes
 ...
 */
export function authenticate(serviceAccount: Record<string, string>): Promise<void> {
 ...
 */
export function buildServiceAccount(): Record<string, string> {
    // Try GOOGLE_CREDENTIALS_JSON first (full JSON)
    const credentialsJson = process.env.GOOGLE_CREDENTIALS_JSON;
    if (credentialsJson) {
        try {
            return JSON.parse(credentialsJson);
        } catch (e) {
            console.warn('[EarthEngine] Failed to parse GOOGLE_CREDENTIALS_JSON');
        }
    }

    // Fall back to individual environment variables
    const privateKey = (process.env.GOOGLE_PRIVATE_KEY || '').replace(/\\n/g, '\n');
    const clientEmail = process.env.GOOGLE_CLIENT_EMAIL || 'wrkfarm-415118@appspot.gserviceaccount.com';
    const projectId = process.env.GOOGLE_PROJECT_ID || 'wrkfarm-415118';

    if (!privateKey) {
        throw new Error(
            'Missing GOOGLE_PRIVATE_KEY. Set it in Vercel environment variables.'
        );
    }
