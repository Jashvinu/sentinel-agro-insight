/**
 * Advanced Monitoring Endpoint for Vercel
...
import { initializeEarthEngine, evaluate, getMapIdWithRetry, ee } from './_lib/earthEngine';
import { geoJsonToEarthEngine, getMergedOpticalCollection } from './_lib/satelliteUtils';
...
async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method !== 'POST') {
        return errorResponse(res, 'Method not allowed. Use POST for analysis.', 405);
    }
        ...
        for (const algorithmId of algorithms) {
            ...
            try {
                ...
                while (windowStart < endMs) {
                    ...
                    const windowCollection = collection.filterDate(windowStartStr, windowEndStr);
                ...
                timeseries.push({ algorithm: algorithmId, windows });
